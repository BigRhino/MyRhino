...
//  HeaderNote
//
//  Created by Rhino on 2017/7/19.
//  Copyright © 2017年 Rhino. All rights reserved.
//

#ifndef NSFileManage_h
#define NSFileManage_h
...
//文件管理器
//1.对文件进行操作(移动,复制,链接,删除文件或目录...)
//2.设置委托,判断是否成功完成,操作失败执行相应的处理操作
...
//判断该指定文件名对应的文件或目录是否存在 最后一个参数:指定文件名是否为目录
...
//获取指定目录下的所有文件和子目录对应的枚举器,如果指定目录下的文件是子目录,该方法默认会递归枚举目录中的所有内容,当然可以调用NSDirectoryEnumerator
//的skipDescendants方法来阻止递归枚举子目录
...
FOUNDATION_EXPORT NSFileAttributeKey const NSFileType;  //文件类型
FOUNDATION_EXPORT NSFileAttributeType const NSFileTypeDirectory; //文件目录
...
FOUNDATION_EXPORT NSFileAttributeKey const NSFileSize;  //文件大小
FOUNDATION_EXPORT NSFileAttributeKey const NSFileModificationDate;  //文件修改时间
...
FOUNDATION_EXPORT NSFileAttributeKey const NSFileCreationDate; //创建时间
FOUNDATION_EXPORT NSFileAttributeKey const NSFileOwnerAccountID; //属主账户
