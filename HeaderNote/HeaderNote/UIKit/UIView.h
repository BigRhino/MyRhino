...
/**
 ...
 @param delay 动画延迟多长时间开始
 @param dampingRatio 阻尼动画的阻尼度 0-1之间,越接近1阻尼度越大
 @param velocity 动画的初速度
...
/**
 切换视图的转场动画:会将fromView从父视图移除,添加toView

 @param fromView 当前视图
 @param toView 需要切换到的视图
 @param duration 动画持续时间
 @param options 动画配置
 @param completion 完成回调
 */
 ...
 */
//在多个视图上执行系统动画(当前只有删除动画),动画效果,持续时间等参数均有系统指定.
...
/**
 ...
 @param delay 动画延迟多久开始(单位:s)
 @param options 动画配置参数
 @param animations 一帧帧的动画内容
 @param completion 完成回调函数
 */
+ (void)animateKeyframesWithDuration:(NSTimeInterval)duration delay:(NSTimeInterval)delay options:(UIViewKeyframeAnimationOptions)options animations:(void (^)(void))animations completion:(void (^ __nullable)(BOOL finished))completion NS_AVAILABLE_IOS(7_0);

//添加一帧动画
///参数:相对开始时间和相对时间间隔,为0-1.0之间的浮点数,表示总的占有比例.
