...
typedef NS_ENUM(NSInteger, UITabBarItemPositioning) {
    ...
    UITabBarItemPositioningFill,      //填充
...
//官方实现:
//items承载着UITabBarItem,相当于数据模型,title,image,selectedImage,badge等等
//UI实现层:背景View(_UIBarBackground:UIView) + 一个个的UITabBarButton:UIControll(UITabBarButtonLabel:UILable,   UITabBarSwappableImageView:UIImageView),badge(UIView)


NS_CLASS_AVAILABLE_IOS(2_0) @interface UITabBar : UIView

//代理 UITabBarController中默认其delegate = UITabBarController控制器实现了,我们更改UITabBarController.tabBar.delegate赋值会崩溃
...
//开始重新排序,即更改items的顺序 调用后在当前对象的父视图上动态的添加一个UICustomizingView:UIView的界面,用于拖拽排序
...
//选中状态下的镂空颜色,(比如选中时的标题文字颜色,系统风格的图标会被渲染,自定义图片不会)
...
//__________________________代理_________________________________________________
//跟UITabBarController的代理方法一样,应该就是给TabbarController使用的.
...
//调用beginCustomizingItems:(NSArray<UITabBarItem *> *)items 会执行下面的代理方法,捕获时机
